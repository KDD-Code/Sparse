...
%   Log Regularized Learning with Calibrated Least Squares Loss.
%
%% OBJECTIVE
%   argmin_W { sum_i^t (l_i(g(w_i)))
%            + rho1 \sum f_i(sigma)}   % f_i(x) = log(x+1)/log(2)
%% INPUT
...
%
%% Code starts here
function [W, funcVal] = TCLog_Nonconvex(X, Y, rho1,opts)
...
while iter < iternum
    ...
    [U S1 V] = mySVD(temp,size(W,1));
    S1 = diag(S1);
    v_temp = funVal_eval(W) + mu*sum(log(sqrt(S1)+1));
    funcVal(iter) = v_temp;
    
    % convergence analysis
    if iter>1
        Vt = funcVal(iter-1);
        if (Vt - v_temp)/Vt<1e-6
            break;            
        end
    end
    iter = iter+1;
